...
def disengagement_distances(eventLocations):
    ...
    for i, segment in enumerate(segments):
        ...
        mode_distance['latitude'] = float(segment[i]['gps']['latitude'])
        mode_distance['longitude'] = float(segment[i]['gps']['longitude'])
...
if __name__ == "__main__":
    ...
    for can in tqdm(canbus_chassis):
        ...
        if(gpsfound is not None):
            canfound = {
                'header': can['header'],
                'drivingMode': can['drivingMode']
            }
            eventLocations['events'].append({'can':canfound,'gps':gpsfound})
            count = count + 1
            tempSave += 1
            if tempSave > 5000:
                # print("DUMPED...")
    ...
    for item in eventLocations['events']:
        gps = item['gps']
    ...
    total_travel_distance   = travel_distance(eventLocations['events'])
    segments, transition_indices = disengagement_distances(eventLocations['events'])
    ...
    fname= data_root+"disengagement_info.json"
    with open(fname,'w') as f:
        json.dump(segments, f, indent=4)
        f.close()
    print("DUMPED TO ", fname)
    print("SCRIPT TIME: ", time.time() - tstart, " seconds")
